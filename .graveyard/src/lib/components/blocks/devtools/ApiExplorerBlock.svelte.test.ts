...
function renderApiExplorer(
	...
	return render(ApiExplorerBlock, {
		target: host,
		props
	});
}
...
describe('ApiExplorerBlock', () => {
	describe('Collapsed State (Ad Mode)', () => {
		...
		it('should call onopen when ad is clicked', async () => {
			const onopen = vi.fn();
			renderApiExplorer({ open: false, onopen });
	...
	describe('Expanded State', () => {
		...
		it('should call onclose when close button is clicked', async () => {
			const onclose = vi.fn();
			renderApiExplorer({ open: true, onclose });
	...
	describe('Endpoints Tab', () => {
		...
		it('should display endpoint descriptions', async () => {
			...
			await expect
				.element(page.getByText('Fetch current debug settings including hero and disco parameters'))
				.toBeVisible();
	...
	describe('Schemas Tab', () => {
		...
		it('should display Hero3DParams schema', async () => {
			...
			await expect.element(page.getByText('cursorShape')).toBeVisible();
			await expect.element(page.getByText('pixelSize')).toBeVisible();
			await expect.element(page.getByText('revealBrightness')).toBeVisible();
		});
	});
	...
	describe('Keyboard Navigation', () => {
		...
		it('should not call onclose on Escape when closed', async () => {
			const onclose = vi.fn();
			renderApiExplorer({ open: false, onclose });
