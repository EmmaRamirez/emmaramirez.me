<script lang="ts">
	...
	const variantClasses: Record<AlertVariant, { container: string; icon: string; title: string }> = {
		...
		warning: {
			container:
				'bg-[var(--transit-yellow-100)] border-[var(--transit-yellow-600)] text-[var(--liver-brown-800)]',
...
<div
	class="flex gap-3 rounded-lg border-l-4 p-4 {variantClasses[variant].container} {className}"
	role="alert"
>
	...
	<div class="min-w-0 flex-1">
		{#if title}
			<h3 class="mb-1 font-semibold {variantClasses[variant].title}">
				{title}
			</h3>
		{/if}
		<div class="text-sm">
