<script lang="ts">
	...
	const emptyAchievementSlots = Array.from({ length: 11 }, (_, index) => index + 1);
	const achievementName = 'Achievement Unlocker';

	let { class: className = '' }: Props = $props();
	let achievementsDialog: HTMLDialogElement | undefined;
	let hasClaimedAchievement = $state(false);
	let isAchievementsDialogOpen = $state(false);
	let isClaimInProgress = $state(false);
	let isClaimInterludeVisible = $state(false);
	let claimInterludeKey = $state(0);
	let claimButtonLabel = $derived(
		isClaimInProgress
			? 'Claiming...'
	...
	function handleClaimButtonClick() {
		...
		if (prefersReducedMotion()) {
			void finishClaimInterlude();
			return;
		}

		claimInterludeKey += 1;
		isClaimInterludeVisible = true;
	}

	async function finishClaimInterlude() {
		if (!isClaimInProgress) {
			return;
		}

		isClaimInProgress = false;
		isClaimInterludeVisible = false;
		hasClaimedAchievement = true;
...
{#if isClaimInterludeVisible}
	{#key claimInterludeKey}
		<div
...
>
	{#if hasClaimedAchievement}
		<div class="achievements-dialog__featured-badge" aria-label={achievementName}>
	...
	<div class="achievements-dialog__panel">
		...
		<div class="achievements-dialog__grid" role="list" aria-label="Achievements">
			{#if hasClaimedAchievement}
				<div
					class="achievements-dialog__achievement achievements-dialog__achievement--unlocked achievements-dialog__achievement--new"
					role="listitem"
