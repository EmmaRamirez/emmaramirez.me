<script lang="ts">
	...
	let cursorHistory = createCursorHistory(discoParams.sampleHistorySize);
	let volatility = 0;
	...
	function createCursorHistory(size: number): CursorHistoryBuffer {
		const safeSize = Math.max(2, size);
		return {
			xs: new Array(safeSize).fill(0),
			ys: new Array(safeSize).fill(0),
			times: new Array(safeSize).fill(0),
			size: safeSize,
			count: 0,
			index: 0
		};
	...
	function ensureCursorHistoryCapacity() {
		const nextSize = Math.max(2, discoParams.sampleHistorySize);
		if (cursorHistory.size === nextSize) return;
		cursorHistory = createCursorHistory(nextSize);
	}
	...
	function calculateVolatility(): number {
		if (cursorHistory.count < 3) return 0;
		...
		let prevDy = 0;
		const start =
			(cursorHistory.index - cursorHistory.count + cursorHistory.size) % cursorHistory.size;
		let previousSampleIndex = start;

		for (let i = 1; i < cursorHistory.count; i++) {
			const currentSampleIndex = (start + i) % cursorHistory.size;
			const dt = Math.max(
				cursorHistory.times[currentSampleIndex] - cursorHistory.times[previousSampleIndex],
				1
			);
			const dx = cursorHistory.xs[currentSampleIndex] - cursorHistory.xs[previousSampleIndex];
			const dy = cursorHistory.ys[currentSampleIndex] - cursorHistory.ys[previousSampleIndex];
		...
		const avgSpeed = totalSpeed / (cursorHistory.count - 1);
		const avgDirectionChange = directionChanges / Math.max(cursorHistory.count - 2, 1);
	...
	function resetCursorHistory() {
		ensureCursorHistoryCapacity();
		cursorHistory.count = 0;
	...
	function updateCursorTracking(x: number, y: number) {
		...
		ensureCursorHistoryCapacity();
		cursorHistory.xs[cursorHistory.index] = x;
		cursorHistory.ys[cursorHistory.index] = y;
		cursorHistory.times[cursorHistory.index] = now;
		cursorHistory.index = (cursorHistory.index + 1) % cursorHistory.size;
		cursorHistory.count = Math.min(cursorHistory.count + 1, cursorHistory.size);
	...
	function updateCursorActiveState() {
		if (!cursorEl) return;
		cursorEl.style.opacity = effectsEnabled && isHovering ? '1' : '0';
	}
	...
	function beginHoverSession() {
		if (!effectsEnabled || isHovering) return;
	...
	function assignCursor(node: HTMLDivElement) {
		cursorEl = node;
		updateCursorActiveState();
		updateCursorVisual();
		return () => {
			if (cursorEl === node) cursorEl = null;
		};
	}
</script>
