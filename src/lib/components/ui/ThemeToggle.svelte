<script lang="ts">
	import { theme } from '$lib/stores';
	import { cn } from '$lib/utils';
	import type { ClassValue } from 'svelte/elements';

	interface ThemeToggleProps {
		class?: ClassValue;
	}

	let { class: className }: ThemeToggleProps = $props();
	let isDark = $derived($theme === 'dark');
</script>

<button
	type="button"
	onclick={() => theme.toggle()}
	class={cn(
		'relative h-8 w-14 rounded-full p-1 transition-all duration-300',
		'border-2 border-(--border-color)',
		'bg-(--page-bg-subtle) hover:bg-(--page-bg-muted)',
		'focus:outline-none focus-visible:ring-2 focus-visible:ring-(--caroline-blue-600)',
		className
	)}
	aria-label={isDark ? 'Switch to light mode' : 'Switch to dark mode'}
>
	<span
		class={cn(
			'absolute top-1 h-5 w-5 rounded-full transition-all duration-300 ease-out',
			'flex items-center justify-center text-sm',
			isDark ? 'translate-x-6 bg-(--text-primary)' : 'translate-x-0 bg-(--liver-brown-700)'
		)}
	>
		{#if isDark}
			<svg class="h-3 w-3 text-(--page-bg)" fill="currentColor" viewBox="0 0 20 20">
				<path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" />
			</svg>
		{:else}
			<svg class="h-3 w-3 text-(--sandy-tan-200)" fill="currentColor" viewBox="0 0 20 20">
				<path
					fill-rule="evenodd"
					d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z"
					clip-rule="evenodd"
				/>
			</svg>
		{/if}
	</span>
</button>
