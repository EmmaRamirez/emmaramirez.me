...
<script lang="ts">
	...
	const typeClasses: Record<ToastType, { bg: string; icon: string; border: string }> = {
		...
		warning: { bg: 'bg-[var(--transit-yellow-100)]', icon: 'text-[var(--transit-yellow-700)]', border: 'border-[var(--transit-yellow-600)]' },
...
<div class="fixed z-[100] flex flex-col gap-2 max-w-sm w-full pointer-events-none {positionClasses[position]} {className}">
