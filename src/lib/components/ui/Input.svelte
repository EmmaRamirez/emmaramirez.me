<script lang="ts">
	...
	const baseInputClasses = "w-full px-3 py-2 rounded-lg border-2 bg-[var(--eggshell-white-500)] text-[var(--liver-brown-800)] placeholder:text-[var(--liver-brown-500)] focus:outline-none focus:ring-2 focus:ring-offset-1 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed";
	...
	const paddingClasses = `${hasLeadingIcon ? 'pl-10' : ''} ${hasTrailingIcon ? 'pr-10' : ''}`;
...
<div class="flex flex-col gap-1.5">
	...
	<div class="relative">
		{#if leadingIcon}
			<div class="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--liver-brown-500)]">
				{@render leadingIcon()}
			</div>
		{/if}
		...
		{#if trailingIcon}
			<div class="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--liver-brown-500)]">
				{@render trailingIcon()}
			</div>
		{/if}
	</div>
